...
```
It doesn't get them all, but it is clearly updating with the new information
...
```
Yeah I think so -- input layer, 2 fully-connected layers with 6 and 2 neurons respectively, and then softmax layer
```
...
```
Yeah it looks like relu is substantially faster.
```
...
```
Summary: changing these parameters can

Details:
Baseline @5000 examples - validation at 0.9
Increasing number of filters: increased accuracy to 0.92
Increasing size of the filters reduced accuracy to 0.85 from baseline

```

* Remove the pooling layers.  Does it impact the accuracy?
```
Compared to the baseline, my accuracy was essentially the same (0.89 compared to baseline of 0.9)
```

* Add one more conv layer.  Does it help with accuracy?
```
Through 5000 examples, it had not helped with accuracy (0.88 on validation).

It is possible that adding this in as I did led to overfitting - so the training accuracy may have been higher, but I am really intersted in the out of sample test.
```

* Increase the batch size.  What impact does it have?
```
Increasing batch size reduced the accuracy (validation)
```
